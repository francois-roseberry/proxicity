(function () {
    ...
    function assertionsOn(rootElement) {
        ...
        return {
            ...
            assertText: function (selector, textContents) {
                checkIsSelector(selector);

				var actualTexts = _.map(rootElement().find(selector), function (item) {
                    return $(item).text();
                });
				
				if (actualTexts.length !== textContents.length) {
					throw new Error('Expected ' + textContents.length + ' elements, but found' +
						actualTexts.length);
				}

                if (_.difference(actualTexts, textContents).length !== 0) {
                    throw new Error("Element " + selector + " should contain text [" +
                        textContents + "] but instead contains [" + actualTexts + "]");
                }
            },
